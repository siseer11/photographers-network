...
export default class GbNavBar extends React.Component {
  ...
  fetchNotifications(user) {
    if (user) {
      this.database.ref('users').child(user.uid).child("notifications")
        .once('value', snap => {
          ...
          this.setState({notifications: notifications, user: user}, () => {
            let unreadNotifications = this.state.notifications.filter(note => !note.read);
  ...
  showNotificationsHandler = () => {
    this.setState(prev => {
      return {showNotificationBox: !prev.showNotificationBox, newNotifications: false}
    });
  };
  ...
  handleReadNotification = id => {
    ...
    this.database.ref("users").child(this.state.user.uid).child("notifications").child(id).update({
      read: true
    }, () => this.setState({notifications: notificationsCopy}));
  };

  render() {
    const {righLinks, loggedIn, userImageUrl, profileLink, homeLink, userOn} = this.props;
            ...
            <React.Fragment>
              {
                ...
                <div onClick={this.showNotificationsHandler} className="bell">
                  <BellSVG classes={`gb-icon-medium gb-icon-fill-white ${newNotifications && 'new'}`}/>
                </div>
              }
              {
                showNotificationBox &&
